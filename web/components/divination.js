  ...
  W.DivinationPage = {
    ...
    computed: {
      pools() {
        ...
        return b.map(p => ({
          gt: String(p.gt), label: (this.a.poolShort[p.gt] || p.gt) + '池',
    ...
    methods: {
      ...
      loadZeri() {
        ...
        fetch('/api/zeri').then(r => r.json())
          .then(r => { if (r && r.ok) this.zeri = r; else this.zeriErr = (r && r.error) || '择时服务未就绪'; })
      ...
      loadBanner() {
        ...
        fetch('/api/banner').then(r => r.json())
          .then(r => {
            if (!r || !r.ok) { this.bannerErr = (r && r.error) || '卡池日历未就绪'; return; }
            this.banner = r;
            this.loadRange();
          })
      ...
      loadRange() {
        ...
        fetch('/api/zeri/range?to=' + encodeURIComponent(to)).then(r => r.json())
          .then(r => { if (r && r.ok) this.range = r; else this.rangeErr = (r && r.error) || '择日服务未就绪'; })
      ...
      luckTx(l) {
        ...
        return (l.kw && l.kw.length) ? t + ' · ' + l.kw.slice(0, 2).join('') : t;
      },
      ...
      async start() {
        ...
        const res = await p;
        if (!res || !res.ok) { this.err = (res && res.error) || '占卜服务未就绪'; }
        ...
        else if (!this.err) this.err = '占卜服务未就绪';
      },
    ...
    <div class="wrap">
      <div class="pagehead">
        <div>
          <h1>八卦占卜</h1>
          <div class="sub">
            当期卡池日期自动获取 · 两套正统起卦法（三枚铜钱 / 大衍揲蓍）· 按朱熹《易学启蒙》变占取用 ·
            卦辞爻辞带注疏与白话 · 剩余期内逐日择吉 ·
            <b>卦只定「要几成把握」，抽数由官方概率模型解出单点</b> —— 不给「1~79」这种用不上的范围
          </div>
        </div>
        <div class="pg-actions">
          <select class="dz-sel dz-sel-s" v-model="method" :disabled="busy" title="起卦法：两种都是正统源流，动爻数分布完全相同">
            <option value="coin">三枚铜钱法</option>
            <option value="yarrow">大衍揲蓍法</option>
          </select>
          <select class="dz-sel dz-sel-pool" v-model="gt" :disabled="busy">
            <option v-for="p in pools" :key="p.gt" :value="p.gt">{{ p.label }}（已垫 {{ p.cur }}）</option>
          </select>
          <select class="dz-sel dz-sel-s" v-if="curPool && curPool.unique" v-model="upMode" :disabled="busy"
                  title="当前保底状态：决定「出金时是当期」的概率">
            <option value="small">小保底</option>
            <option value="big">大保底</option>
          </select>
          <button class="btn" v-if="busy" @click="fastForward">跳过动画</button>
          <button class="btn" v-if="phase==='done'" @click="reset">重新起卦</button>
          <button class="btn pri" v-else-if="!busy" @click="start">开始摇卦</button>
        </div>
      </div>
      ...
      <div class="dz-bn" v-if="bnTerm">
        <div class="dz-bn-main">
          <span class="dz-bn-tag">当期卡池</span>
          <b class="dz-bn-ver">v{{ bnTerm.version }}{{ bnTerm.half ? ' · ' + bnTerm.half : '' }}</b>
          <span class="dz-bn-date">{{ bnTerm.start }} ~ {{ bnTerm.end }}</span>
          <span class="dz-bn-rm" :class="{ hot: banner.remain.days <= 2 }">剩余 {{ banner.remain.text }}</span>
          <span class="dz-bn-cal" v-if="bnCalibrated" title="开池时刻与本地抽卡记录的首抽时间对得上">已校准</span>
        </div>
        <div class="dz-bn-ups">
          <span v-if="bnTerm.upChars.length"><i>UP 角色</i>{{ bnTerm.upChars.join(' / ') }}</span>
          <span v-if="bnTerm.upCones.length"><i>UP 光锥</i>{{ bnTerm.upCones.join(' / ') }}</span>
        </div>
        <div class="dz-bn-note">
          {{ banner.note }}<template v-if="bnCalibrated"> · 开池时刻已用本地抽卡记录校准</template>
          <template v-if="banner.source === 'builtin'"> · 当前用的是内置表（联网取不到）</template>
          <template v-if="banner.stale"> · 缓存已过期，显示的是上次结果</template>
        </div>
      </div>
      <div class="note dz-err" v-if="bannerErr">卡池日历取不到：{{ bannerErr }}　（其余功能不受影响）</div>
      ...
      <div class="dz-top">
        ...
        <div class="dz-side">
          <div class="dz-card">
            <div class="dz-card-h">
              <b>六爻</b><span class="dz-mini">自下而上：第一次 = 初爻</span>
            </div>
            <div class="dz-hex">
              <div v-for="row in hexRows" :key="row.i" class="dz-hrow" :class="{ empty: row.empty, moving: row.moving }">
                ...
                <span class="dz-hmov">{{ row.empty ? '' : (row.moving ? (row.value === 9 ? '○ 老阳' : '✕ 老阴') : '静') }}</span>
              </div>
            </div>
            <div class="dz-legend">
              <template v-if="method === 'yarrow'">
                <span>四十九策 · 三变一爻</span>
                <span>老阳 3/16 · 少阴 7/16 · 少阳 5/16 · 老阴 1/16</span>
              </template>
              <template v-else>
                <span><i class="dot gold"></i>背 = 阳</span>
                <span><i class="dot dim"></i>字 = 阴</span>
              </template>
              <span>○ 老阳 · ✕ 老阴 = 动爻</span>
            </div>
          </div>
          ...
          <div class="dz-card" v-if="done.length">
            <div class="dz-card-h"><b>{{ method === 'yarrow' ? '三变一爻 · 明细' : '三次一爻 · 明细' }}</b></div>
            <table class="dz-tbl">
              <tbody>
                ...
                <template v-for="row in hexRows" :key="row.i">
                  <tr v-if="!row.empty">
                    ...
                    <td class="v">
                      ...
                      <span v-else class="dz-coinmini">{{ row.gone ? row.gone.join('+') + ' → 余 ' + row.remain : '—' }}</span>
                    </td>
      ...
      <!-- ── 今日吉时（常驻：进页就能看，不必先摇卦） ──────────────────── -->
      <h2>今日吉时 <small>时家择吉 · 十二时辰排序 · 万年历</small></h2>
      <div class="dz-card dz-zt">
        <div class="dz-err" v-if="zeriErr">择时数据取不到：{{ zeriErr }}</div>
        <template v-else-if="zeri">
          <div class="dz-zt-top">
            ...
            <div class="dz-zt-now" v-if="nowHour">
              此刻 <b>{{ nowHour.zhi }}时</b> · {{ nowHour.label }}
            </div>
          </div>

          <div class="dz-zt-best" :class="'lv-' + bestHour.level">
            <span class="k">{{ bestHour.level === 's1' ? '今日最宜' : '今日无吉时 · 退求其次' }}</span>
            <span class="v">{{ bestHour.zhi }}时 {{ bestHour.range }}</span>
            <span class="s">{{ bestHour.tianShen }}·{{ bestHour.type }}{{ bestWhy ? ' · ' + bestWhy : '' }}　得分 {{ bestHour.score > 0 ? '+' : '' }}{{ bestHour.score }}</span>
          </div>
          ...
          <div class="dz-zt-legend">
            <span><i class="sw lv-s1"></i>吉时</span>
            <span><i class="sw lv-s2"></i>次吉</span>
            <span><i class="sw lv-s0"></i>平常</span>
            <span><i class="sw lv-sx"></i>凶时</span>
            <span class="sp">按十二时辰自然序排列 · 描边为此刻所在时辰</span>
          </div>

          <table class="dz-tbl dz-zt-tbl">
            <thead>
              <tr><th>#</th><th>时辰</th><th>时段</th><th>吉凶依据</th><th>分</th><th>判定</th></tr>
            </thead>
            <tbody>
              <tr v-for="h in zeri.hours" :key="h.zhi">
                ...
                <td class="lb"><span class="dz-lg" :class="'lv-' + h.level">{{ h.label }}</span></td>
              </tr>
            </tbody>
          </table>

          <div class="dz-zt-yiji" v-if="(zeri.day.yi && zeri.day.yi.length) || (zeri.day.ji && zeri.day.ji.length)">
            <div><span class="k">宜</span><span class="v yi">{{ (zeri.day.yi || []).slice(0, 9).join('、') || '无' }}</span></div>
            <div><span class="k">忌</span><span class="v ji">{{ (zeri.day.ji || []).slice(0, 9).join('、') || '无' }}</span></div>
            <div><span class="k">日吉神</span><span class="v">{{ (zeri.day.jiShen || []).join('、') || '无' }}</span></div>
            <div><span class="k">日凶煞</span><span class="v bad">{{ (zeri.day.xiongSha || []).join('、') || '无' }}</span></div>
          </div>

          <details class="dz-more">
            <summary>择时口径（4 个维度 + 档位阈值）—— 点开可逐条复核</summary>
            <ul class="dz-ul">
              <li v-for="d in zeri.doc.dims" :key="d.k"><b>{{ d.k }}</b> — {{ d.v }}</li>
              <li><b>档位阈值</b> — {{ zeri.doc.levels }}</li>
              <li><b>升档与降档</b> — {{ zeri.doc.upgrade }}</li>
              <li><b>子时口径</b> — {{ zeri.doc.note }}</li>
              <li><b>免责</b> — {{ zeri.doc.duty }}</li>
            </ul>
          </details>
        </template>
      </div>
      ...
      <template v-if="range">
        <h2>剩余期内择日 <small>日家择吉 · 只在当期卡池剩下的 {{ range.n }} 天里排</small></h2>
        <div class="dz-card dz-rg">
          <div class="dz-rg-top">
            <span class="dz-mini">扫描区间 {{ range.start }} ~ {{ range.end }}（{{ range.n }} 天）</span>
            <span class="dz-mini" v-html="bold(range.doc.combine)"></span>
          </div>
          <div class="dz-rg-cut" v-if="rangeCut">
            卡池于 <b>{{ rangeCut.end }}</b> 关闭<template v-if="rangeCut.partial"> —— 到期当天只开到凌晨，
            之后的时辰没有池子，<b>因此不作推荐</b>（只排到 {{ rangeCut.to }}）</template>。
          </div>
          <table class="dz-tbl dz-rgt">
            <thead>
              <tr><th>名次</th><th>日期</th><th>日辰</th><th>日家</th><th>最佳时辰</th><th>吉时</th><th>综合</th></tr>
            </thead>
            <tbody>
              <tr v-for="d in range.ranked.slice(0, 6)" :key="d.date" :class="{ best: d.rank === 1 }">
                ...
                <td class="lb">
                  <span class="dz-lg" :class="dgCls(d)">{{ d.label }}</span>
                  <span v-for="(m,k) in d.marks" :key="k" class="dz-mk" :class="{ neg: m.d < 0 }">{{ m.tag }}</span>
                </td>
                <td class="k">{{ d.bestHour.zhi }}时<small>{{ d.bestHour.range }} · {{ d.bestHour.label }}</small></td>
                <td class="r">{{ d.hoursGood }} 个</td>
                <td class="sc">{{ d.total > 0 ? '+' : '' }}{{ d.total }}</td>
              </tr>
            </tbody>
          </table>
          <details class="dz-more" v-if="range.ranked.length > 6">
            <summary>其余 {{ range.ranked.length - 6 }} 天的完整排序 —— 点开</summary>
            <table class="dz-tbl dz-rgt">
              <tbody>
                <tr v-for="d in range.ranked.slice(6)" :key="d.date">
                  ...
                  <td class="lb"><span class="dz-lg" :class="dgCls(d)">{{ d.label }}</span>
          ...
          <details class="dz-more">
            <summary>日家打分口径（{{ range.doc.dims.length }} 个维度 + 档位）—— 点开可逐条复核</summary>
            <ul class="dz-ul">
      ...
      <div class="note dz-err" v-if="rangeErr">剩余期内择日取不到：{{ rangeErr }}</div>

      <div class="note dz-err" v-if="err">{{ err }}　（动画里的卦象仅作演示，解读需要服务启动）</div>

      <!-- ── 结果 ──────────────────────────────────────────────────────── -->
      <template v-if="result">
        <h2>卦象 <small>本卦定当下 · 动爻是破局点 · 变卦看趋势</small></h2>
        <div class="dz-gua">
          <div class="dz-gcard">
            <div class="dz-gtag">本卦</div>
            <div class="dz-gname">{{ result.ben.full }}</div>
            <div class="dz-gsym">
              <span>{{ trigram[result.ben.shang] }}</span><span>{{ trigram[result.ben.xia] }}</span>
            </div>
            <div class="dz-gsub">第 {{ result.ben.no }} 卦 · 上{{ result.ben.shang }} 下{{ result.ben.xia }}</div>
            <div class="dz-gci">{{ result.ben.ci }}</div>
            <div class="dz-gxiang" v-if="result.ben.xiang">象曰：<span v-html="bold(result.ben.xiang)"></span></div>
            <div class="dz-gsay" v-if="result.ben.say"><i>白话</i><span v-html="bold(result.ben.say)"></span></div>
            <details class="dz-more dz-gdet" v-if="result.ben.tuan || result.ben.za || result.ben.yong">
              <summary>更多注疏</summary>
              <ul class="dz-ul">
                <li v-if="result.ben.tuan"><b>彖传</b> — <span v-html="bold(result.ben.tuan)"></span></li>
                <li v-if="result.ben.za"><b>杂卦传</b> — <span v-html="bold(result.ben.za)"></span></li>
          ...
          <div class="dz-gcard" :class="{ alt: result.moving.length }">
            <div class="dz-gtag">变卦</div>
            <div class="dz-gname">{{ result.bian.full }}</div>
            <div class="dz-gsym">
              <span>{{ trigram[result.bian.shang] }}</span><span>{{ trigram[result.bian.xia] }}</span>
            </div>
            <div class="dz-gsub">
              第 {{ result.bian.no }} 卦 · {{ result.moving.length ? '动爻 ' + result.movingNames.join('、') + ' 反转而得' : '六爻皆不动，与本卦相同' }}
            </div>
            <div class="dz-gci">{{ result.bian.ci }}</div>
            <div class="dz-gxiang" v-if="result.bian.xiang">象曰：<span v-html="bold(result.bian.xiang)"></span></div>
            <div class="dz-gsay" v-if="result.bian.say"><i>白话</i><span v-html="bold(result.bian.say)"></span></div>
            <details class="dz-more dz-gdet" v-if="result.bian.tuan || result.bian.za">
              <summary>更多注疏</summary>
              <ul class="dz-ul">
                <li v-if="result.bian.tuan"><b>彖传</b> — <span v-html="bold(result.bian.tuan)"></span></li>
                <li v-if="result.bian.za"><b>杂卦传</b> — <span v-html="bold(result.bian.za)"></span></li>
              </ul>
            </details>
          </div>
        </div>

        <h2>断卦 <small>{{ result.rule.text }}</small></h2>
        <div class="dz-card">
          <div class="dz-cite" v-for="(c,k) in result.cited" :key="k" :class="{ main: c.primary }">
            <div class="dz-cite-h">
              <span class="dz-badge">{{ c.primary ? '主判据' : '参考' }}</span>
              <b>{{ c.kind }}</b><span class="dz-lbl">{{ c.label }}</span>
            </div>
            <div class="dz-cite-t">{{ c.text }}</div>
            <div class="dz-var" v-if="c.variants"><b>异文</b><span v-for="(v,j) in c.variants" :key="j">{{ v }}</span></div>
          </div>
          <div class="dz-slim">
            动爻共 <b>{{ result.moving.length }}</b> 个{{ result.moving.length ? '（' + result.movingNames.join('、') + '）' : '' }}；
            取用规则出自朱熹《易学启蒙 · 考变占》，0~6 个动爻各有定法，不由本平台自创。
          </div>
        </div>

        <h2>今日运势 <small>卦象字面吉凶 · 时辰倾向</small></h2>
        <div class="dz-2col">
          <div class="dz-card">
            <div class="dz-luck">
              <div class="dz-lv" :class="'lv-' + result.luck.level">{{ result.luck.label }}</div>
              <div class="dz-lvtx">
                <b>{{ result.luck.action }}</b>
                <span>得分 {{ result.luck.total }} = 主判据 {{ result.luck.main }} + 本卦卦辞 {{ result.luck.ben }} + 变卦卦辞 {{ result.luck.bian }}</span>
              </div>
            </div>
            <div class="dz-words" v-if="result.luck.mainWords.length">
              主判据命中：<b v-for="(w,k) in result.luck.mainWords" :key="k">{{ w }}</b>
            </div>
            <div class="dz-slim">
              判定方式是把引文里的吉凶字眼按一张固定的词表打分管，<b>命中的字词全部列在上方</b>，
              可逐条复核。档位阈值按 12000 次抽样的分数分布分位数定，所以「大吉」约占一成、不是随手给的好话。
            </div>
          </div>
          <!-- 万年历（日家/时家/宜忌）已移到上方常驻的「今日吉时」卡里，
               这里只留「卦与时的关系」——两张卡不重复同一份信息 -->
          <div class="dz-card">
            <div class="dz-card-h"><b>择时</b><span class="dz-mini">卦定宜进宜守 · 时辰另算</span></div>
            <div class="dz-alm" v-if="bestHour">
              <div><span class="k">今日最宜</span><span class="v">{{ bestHour.zhi }}时 {{ bestHour.range }}（{{ bestHour.label }}{{ bestWhy ? ' · ' + bestWhy : '' }}）</span></div>
              <div v-if="nowHour"><span class="k">此刻</span><span class="v">{{ nowHour.zhi }}时 — {{ nowHour.label }}{{ nowHour.level === 's1' ? '，正当吉时' : '' }}</span></div>
              <div v-if="result.almanac.hour"><span class="k">摇卦时</span><span class="v">{{ result.almanac.hour.zhi }}时 {{ result.almanac.hour.gz }}（{{ result.almanac.hour.type }} · {{ result.almanac.hour.tianShen }}）</span></div>
            </div>
            <div class="dz-slim">
              十二时辰的完整吉凶榜与打分依据见上方「今日吉时」。
              <b>卦与时辰互不改数</b>：时辰只回答「什么时候出手」，出手抽多少仍由保底模型给出。
            </div>
            <div class="dz-slim" v-if="!bestHour && zeriErr">择时数据取不到：{{ zeriErr }}</div>
          </div>
        </div>

        <h2>时机推演 <small>卦定把握度 · 数学定抽数 · 给单点不给范围</small></h2>
        <div class="dz-card">
          <div class="dz-tmg">
            <div class="dz-tmg-l">
              <div class="dz-tmg-k">
                {{ headline.kind === 'up' ? '拿当期限定 · 把握线' : '出金线 · 建议出手点' }}
              </div>
              <div class="dz-tmg-v">{{ headline.at }} <small>本池第几抽</small></div>
              <div class="dz-tmg-s">
                还需 <b>{{ headline.draws }}</b> 抽 · 实际把握 <b>{{ pct(headline.chance) }}</b>
              </div>
              <div class="dz-tmg-s">{{ headline.note }}</div>
              <div class="dz-tmg-s">
                卦象档位 <b>{{ result.timing.goalNote }}</b>（目标 {{ pct(result.timing.goal) }}）。
                实际落点会略高于目标 —— 软保底区一抽能跳好几个百分点，停不到整数上。
              </div>
            </div>
            <div class="dz-tmg-r">
              <div class="dz-tblrow" v-for="l in lines" :key="l.key">
                <span class="k">{{ l.name }}<small>{{ l.tag }}</small></span>
                <span class="v">
                  <b>{{ l.reached ? '本池第 ' + l.at + ' 抽' : '需跨期 ' + l.draws + ' 抽' }}</b>
                  <span v-if="l.reached">（还需 {{ l.draws }}）</span>
                  · 把握 {{ pct(l.chance) }}<em v-if="!l.reached" class="dz-note">（跨期累计）</em>
                  <em v-if="!l.reached" class="dz-warn">超出本期！本期最多 {{ pct(l.maxChance) }}</em>
                </span>
              </div>
              <div class="dz-tblrow"><span class="k">起卦法</span><span class="v">{{ result.method.name }} · {{ result.method.how }}</span></div>
              <div class="dz-tblrow"><span class="k">卡池</span><span class="v">{{ result.timing.pool }}{{ result.timing.approx ? '（官方未公示软保底，按基础概率 + 硬保底建模）' : '' }}</span></div>
              <div class="dz-tblrow"><span class="k">已垫 / 硬保底</span><span class="v">{{ result.timing.cur }} / {{ result.timing.hard }} · 还剩 {{ result.timing.remain }} 抽必出</span></div>
              <div class="dz-tblrow" v-if="result.timing.unique">
                <span class="k">保底状态</span>
                <span class="v">{{ result.timing.upMode === 'big' ? '大保底 · 出金必定是当期' : '小保底 · 出金有 ' + (result.timing.pUp * 100).toFixed(0) + '% 是当期' }}<em v-if="result.timing.upEstimated" class="dz-note">（本池官方未单独公示 UP 率，按同类池口径估）</em></span>
              </div>
              <div class="dz-tblrow"><span class="k">平均还需</span><span class="v">{{ result.timing.mean.toFixed(1) }} 抽出金（数学期望）· 半数人 {{ result.timing.ref.p50 }} 抽内出金</span></div>
              <div class="dz-tblrow" v-if="result.timing.unique">
                <span class="k">稳拿所需</span><span class="v">{{ result.timing.upSafe }} 抽 —— 小保底最坏要「歪一次 + 再吃满一个保底」，会跨到下一期</span>
              </div>
            </div>
          </div>
          ...
          <div class="dz-curve" v-if="curve">
            ...
            <div class="dz-curve-l">
              未来 {{ result.timing.remain }} 抽的累计把握曲线：实线 = 出金<span v-if="result.timing.unique">，虚线 = 拿到当期限定</span>；竖线 = 主推的第 {{ headline.at }} 抽
            </div>
          </div>

          <table class="dz-tbl dz-anc">
            <thead>
              <tr><th>本池第几抽</th><th>还需</th><th>该抽单抽概率</th><th>累计把握</th><th>刻度</th></tr>
            </thead>
            <tbody>
          ...
          <div class="dz-slim">
            <b>为什么给单点，不给范围</b>：范围（比如「1~79 抽」）对决策没用 —— 它把「第 1 抽就出」和
            「第 79 抽才出」并列成同一件事。这里的做法是：卦象先定「几成把握算够」（大吉六成 → 凶等硬保底），
            再由官方概率模型解出<b>最小的那一抽</b>，并把这一抽的<b>真实把握</b>一起给出。
          </div>
          <div class="dz-slim">
            <b>概率从哪来</b>：官方只公示「基础概率 + 综合概率 + 硬保底」，没公示软保底曲线。本平台用的曲线
            （角色 74 抽起每抽 +6%、光锥 66 抽起每抽 +7%）<b>回算综合概率 1.605% / 1.872%</b>，
            与官方公示的 1.600% / 1.870% 吻合。整条累计曲线已用 <b>20 万次蒙特卡洛</b>对账，
            解析值与模拟值差在 0.2 个百分点以内。<b>卦不会凭空多给你运气</b>，它只决定你要求几成把握才出手。
          </div>
        </div>

        <h2>逐爻明细 <small>{{ method === 'yarrow' ? '十八变的归奇与余策' : '三枚铜钱的正反' }} · 与对应爻辞</small></h2>
        <div class="dz-card">
          <table class="dz-tbl wide">
            <thead>
              <tr><th>爻位</th><th>{{ method === 'yarrow' ? '三变归奇' : '三枚铜钱' }}</th><th>爻象</th><th>爻辞</th><th>吉凶</th><th>解释 · 白话</th></tr>
            </thead>
            <tbody>
              <tr v-for="row in hexRows" :key="row.i" :class="{ moving: row.moving }">
                ...
                <td class="r">{{ row.name }}{{ row.moving ? ' · 动' : '' }}</td>
                <td class="ct">
                  <div class="dz-ciw">{{ row.benCi }}
                    <div class="dz-ylc" v-if="row.bianCi && row.bianCi !== row.benCi">变卦：{{ row.bianCi }}</div>
                  </div>
                </td>
                <td class="r"><span class="dz-lg" :class="luckCls(row.benLuck)">{{ luckTx(row.benLuck) }}</span></td>
                <td class="ct">
                  <div class="dz-xs" v-if="row.benXiang"><i>小象</i>{{ row.benXiang }}</div>
                  <div class="dz-xs say" v-if="row.benSay"><i>白话</i><span v-html="bold(row.benSay)"></span></div>
                  <div class="dz-xs" v-if="row.bianXiang"><i>变爻</i>{{ row.bianXiang }}</div>
                  <div class="dz-xs say" v-if="row.bianSay"><i>变白话</i><span v-html="bold(row.bianSay)"></span></div>
        ...
        <div class="dz-card">
          <ul class="dz-ul">
            <li><b>起卦法一 · 三枚铜钱</b>：<b>背为阳</b>（一背二字「单·少阳」／二背一字「拆·少阴」／三背「重·老阳」／三字「交·老阴」）。概率由三枚公平硬币决定：少阳 3/8、少阴 3/8、老阳 1/8、老阴 1/8 —— 不是四种各 1/4。</li>
            <li><b>起卦法二 · 大衍揲蓍</b>：四十九策，分二·挂一·揲四·归奇，三变一爻、十八变一卦。四象概率 老阳 3/16、少阴 7/16、少阳 5/16、老阴 1/16（归奇只出 13/17/21/25，余策 36/32/28/24 除以 4）。⚠️ 网上不少页面把少阳与少阴写成 7/16 和 5/16，那是错的 —— 本平台这组数用 60 万次抽样钉死，实测 18.71% / 43.79% / 31.24% / 6.26%。</li>
            <li><b>两法的关系</b>：两法的 P(该爻为动爻) 都是 1/4，所以 <b>动爻数与变卦的分布完全相同</b>（0~6 个动爻的概率都与 B(6, ¼) 吻合，实测偏差小于 0.5 个百分点）。差别只在「动的那一爻是阳变阴还是阴变阳」—— 大衍法 3:1 偏阳变阴，铜钱法 1:1。<b>换起卦法不会让卦象变多或变少</b>。</li>
            <li><b>成卦</b>：六爻<b>自下而上</b>排列，第一次掷的落在初爻。老阳/老阴为动爻，动爻阴阳反转得变卦。</li>
            <li><b>取用</b>：朱熹《易学启蒙 · 考变占》按动爻数 0~6 决定读本卦卦辞、动爻爻辞还是变卦卦辞，以上爻/下爻为主也有定法。</li>
            <li><b>引文</b>：卦辞与爻辞取自三个互相独立的开源数据集，归一化爻序与繁简后<b>多数票定稿</b>：卦辞 61/64、爻辞 362/384 三源逐字一致；有异文的条目在上方标注了各源原文。</li>
            <li><b>吉凶</b>：字面词表打分，不是卦义阐释；命中的词全部列出，可自行复核。档位阈值按 12000 次抽样的分位数标定，不是手拍。</li>
            <li><b>抽数为什么是单点</b>：全部来自官方公示概率模型，与卦象无关。卦象只决定<b>「要几成把握才出手」</b>（大吉六成 → 凶等硬保底），抽数是解出的<b>最小达标那一抽</b>。软保底区一抽能跳好几个百分点，所以实际把握会略高于目标值 —— 这是离散分布的性质，不是算宽了。</li>
            <li><b>出金线 vs UP 线</b>：出金 = 任何 5★；UP 线 = 当期限定，且把「先歪一次、再吃大保底」整段算进去。小保底时 UP 线可能<b>超出本期</b>（最坏要歪一次再吃满一个保底）；这时页面直接摊开本期天花板，而不是给一个根本做不到的数字。</li>
            <li><b>准确性</b>：概率部分（「第 N 抽」、UP 线、稳拿抽数）已用 <b>20 万次蒙特卡洛</b>对账，解析值与模拟值差在 0.2 个百分点以内，「稳拿」抽数经 2 万次模拟无一失手。⚠️ 但这些精确性<b>全部来自游戏的概率系统，不是占卜的预测力</b> —— 卦只提供「几成把握算够」这把尺子。</li>
            <li><b>卡池日期</b>：当期卡池的起止与 UP 名单来自第三方日历（api.ennead.cc），<b>不是官方接口</b>。数据源给的开始时刻偏 +7 小时，已用本地抽卡记录的「首抽时间」校准到开池当日中午；如与游戏内公告不符，<b>以游戏内为准</b>。这是本页唯一的联网请求，其余功能全部离线可用。</li>
            <li><b>解释的四个层次</b>：彖传（释卦辞）· 大象传（释卦象）· 小象传（逐爻释爻辞）· 吉/中/凶标签（384 爻全有）—— 前三者取自古籍与开源数据集，可查证；<b>「白话」一栏是本平台撰述</b>，不是原文，用来自查意思有没有读反。</li>
            <li><b>剩余期内择日</b>：⚠️ 黄历里没有「宜抽卡」，本页把它映射到最接近的「求财 / 开市 / 纳财 / 交易」—— <b>这个映射是本平台定的，不是古法</b>。日家打分只用《协纪辨方书》体系里的黄黑道、建除十二神、二十八宿、宜忌与吉神凶煞，档位阈值按分位数标定。择吉是传统口径，<b>与抽卡概率无关</b>。</li>
            <li>⚠️ 本页是<b>娱乐与自我参照</b>用途，不构成任何消费建议。抽卡结果由游戏的概率系统决定，与占卜无关。保底状态需要你自己确认 —— 记录里没有「上一个金是不是当期」的可靠字段，硬猜就是循环论证。</li>
          </ul>
        </div>
      </template>
    </div>
