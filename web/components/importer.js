...
//
// 为什么要有这块：
//   「角色管理」页的星魂 / 叠影原本只靠「接口窗口 ＋ 截图补录」两源合计，
//   两源都只覆盖接口能回溯的、以及当年手抄下来的那部分 —— 会更早的历史之外还缺。
//   而你在游戏内角色列表 / 工坊统计页 / 米游社上看到的，正是账号**此刻的真实持有状态**，
//   比用抽卡记录反推更可信。这个功能就是把那份状态搬进来。
//
// 流程：上传（原图留档）→ 浏览器里用本机图标库做模板匹配 → **人工确认表**（改错 / 增删）
//       → 保存 → 引擎按「真值快照、覆盖旧值」合并 → 角色管理页立刻更新 + 缺失图标自动补齐
//
// ⚠️ 识别只是「把表预填好」，不保证全对：截图分辨率、缩放、UI 皮肤都会影响命中率。
//    所以确认表是必经环节，宁可标「未识别」也不要猜错。
  ...
  W.ShotImport = {
    ...
    setup(props, { emit }) {
      ...
      const loadAll = async () => {
        ...
        } catch (e) { err.value = '读取图标库 / 外部统计失败：' + e.message; }
      };

      // ── 上传 + 识别 ────────────────────────────────────────────────────────
      const onPick = async (files) => {
        const list = [...files].filter(f => /^image\//.test(f.type) || /\.(png|jpe?g|webp)$/i.test(f.name));
        if (!list.length) { err.value = '请选 PNG / JPG / WebP 图片'; return; }
        if (!lib.value) { err.value = '图标库还没载入完，稍等一下再试'; return; }
        busy.value = true; err.value = ''; okMsg.value = ''; bad.value = []; prog.value = { msg: '准备…', pct: 0 };
        try {
          for (const f of list) {
            // ① 原图留档（服务端只存不解析）
            prog.value = { msg: '上传留档 ' + f.name + '…', pct: 1 };
            ...
            prog.value = { msg: '识别完成：' + r.rows.length + ' 个区域（' + r.meta.ms + 'ms，工作尺寸 ' + r.meta.work + '）', pct: 100 };
          }
          okMsg.value = '已上传 ' + list.length + ' 张，识别结果已填进下面的确认表 —— 请核对后再保存';
          dedupe();
          emit('refetch');
        } catch (e) { err.value = '处理失败：' + e.message; }
      ...
      const clearRows = () => { rows.value = []; okMsg.value = ''; err.value = ''; bad.value = []; };

      // ── 保存 ───────────────────────────────────────────────────────────────
      const save = async () => {
        err.value = ''; okMsg.value = ''; bad.value = []; dups.value = [];
        if (!rowsValid.value.length) { err.value = '确认表里还没有有效行（每行都要有类型和名字）'; return; }
        const at = String(snapAt.value || '').trim().replace('T', ' ') || fmtNow();
        if (!SNAP_RE.test(at)) { err.value = '「快照时间」格式应为 2026-09-17 或 2026-09-17 09:07:12'; return; }
        busy.value = true;
        try {
          ...
          saved.value = await (await fetch('/api/external')).json();
          const ic = j.icons || {};
          okMsg.value = '已保存 ' + j.items.length + ' 条（角色 ' + j.items.filter(x => x.kind === 'ch').length
            + ' / 光锥 ' + j.items.filter(x => x.kind === 'lc').length + '）'
            + (ic.downloaded ? '，自动补齐了 ' + ic.downloaded + ' 个图标' : '，图标都已存在')
            + ' —— 角色管理页已更新';
          rows.value = [];
          emit('refetch');
      ...
      const clearAll = async () => {
        if (!confirm('清空全部外部统计？角色管理页会退回到「接口窗口 ＋ 截图补录」两源的推算值。')) return;
        ...
        try {
          ...
          okMsg.value = '已清空外部统计';
          emit('refetch');
        } catch (e) { err.value = '清空失败：' + e.message; }
        finally { busy.value = false; }
      };
      ...
      const snapHint = computed(() => {
        ...
        return v.length <= 10 ? '已手动填到「日」' : '已手动填写，不再跟随';
      });
      ...
      const boxesOf = file => {
        ...
        return rows.value.filter(r => r.shot === file && r.size).map(r => ({
          ...
          title: (r.name || '未识别') + (r.score != null ? ' · ' + r.score.toFixed(3) : ''),
        }));
      };
    ...
    <div class="imp">
      <div class="ch">
        除官方抽卡接口之外，你在<b>别处看到的总量/持有状态</b>也能录进来：游戏内「角色 / 光锥」列表、星穹工坊统计页、米游社等。
        上传截图 → 平台用<b>本机图标库</b>做模板匹配（<b>全程离线，图片不出本机</b>）→ 你在下面确认表里核对/改正 → 保存。
        <br>它只提供<b>星魂 / 叠影</b>，所以<b>不参与任何抽数口径</b>（总抽数 / 出金率 / 每 UP / 小保底不歪都不受影响）；
        与抽卡记录推算不一致时<b>以这里为准</b>，但会明确提示冲突在哪。
      </div>
      ...
      <div class="imp-drop" :class="{ busy }" @dragover.prevent @drop="onDrop"
        ...
        <div class="di">⇪</div>
        <div class="dt"><b>把截图拖进来</b>，或点这里选择文件（可多选）</div>
        <div class="ds">支持 PNG / JPG / WebP · 单张上限 24MB · 原图会留档在 <code>data/uploads/</code>，方便事后回溯</div>
      </div>

      <div class="row imp-opt">
        <label class="imp-ck"><input type="checkbox" v-model="fivesOnly"> 只匹配五星（更快，角色管理页本来就只统计五星）</label>
        <label class="imp-ck">灵敏度
          <input type="range" min="0.2" max="1.8" step="0.1" v-model.number="sensitivity">
          <b>{{ sensitivity.toFixed(1) }}</b>
        </label>
        <label class="imp-ck"><input type="checkbox" v-model="drawBoxes"> 在预览上画出识别框</label>
      </div>
      ...
      <div v-if="okMsg" class="badge-ok" style="font-size:12.5px">{{ okMsg }}</div>
      ...
      <!-- ── 人工确认表 ──────────────────────────────────────────────────── -->
      <h3 style="margin-top:18px">确认表 <small v-if="rows.length">{{ rows.length }} 行 · 识别结果仅供参考，请逐行核对</small></h3>
      <div class="ch">
        带「<b>识别</b>」角标的行是自动填的，<b>分数</b>越接近 1 越可信（<b>≥0.86 较稳</b>、0.72~0.86 要留意、<b>&lt;0.72 建议手动改</b>）。
        角色填<b>星魂</b>（0~6），光锥填<b>叠影</b>（1~5，只抽到 1 张就填 1）。改完点最下面的「保存到角色管理」。
      </div>
      <table class="tb imp-tb">
        <thead><tr>
          <th style="width:52px">图</th><th style="width:96px">类型</th><th>名字</th>
          <th class="hn" style="width:96px">星魂/叠影</th><th class="hn" style="width:76px">分数</th><th style="width:44px"></th>
        </tr></thead>
        <tbody>
          <tr v-for="(r, i) in rows" :key="i">
            ...
            <td>
              <select v-model="r.kind">
                <option value="ch">角色</option>
                <option value="lc">光锥</option>
              </select>
              <span v-if="r.auto" class="tag-auto">识别</span>
            </td>
            <td>
              <input :list="r.kind === 'lc' ? 'lc-names' : 'ch-names'" v-model="r.name" placeholder="输入名字（可搜索）">
            </td>
            <td class="num"><input type="number" class="vnum" v-model.number="r.v" :min="r.kind === 'lc' ? 1 : 0" :max="r.kind === 'lc' ? 5 : 6"></td>
            <td class="num"><span v-if="r.score != null" class="sc" :class="scoreCls(r.score)">{{ r.score.toFixed(3) }}</span><span v-else class="sc na">手动</span></td>
            <td><button class="btn xs" @click="delRow(i)" title="删掉这一行">✕</button></td>
          </tr>
          <tr v-if="!rows.length"><td colspan="6" style="color:var(--tx3)">还没有待确认的行 —— 上传截图，或点下面「＋ 手动加一行」。</td></tr>
        </tbody>
      </table>
      <datalist id="ch-names"><option v-for="n in nameList('ch')" :key="n" :value="n"></option></datalist>
      <datalist id="lc-names"><option v-for="n in nameList('lc')" :key="n" :value="n"></option></datalist>

      <div class="row">
        <button class="btn" @click="addRow">＋ 手动加一行</button>
        <button class="btn" :disabled="!rows.length" @click="clearRows">清空确认表</button>
        <span class="sep"></span>
        <label class="fl2">快照时间</label>
        <input class="snap" type="text" v-model="snapAt" @input="snapTouched = true" placeholder="2026-09-17 09:07:12">
        <button v-if="snapTouched" class="lnk" type="button" @click="followNow">↻ 跟随当前时间</button>
        <span class="hint2">{{ snapHint }}</span>
      </div>
      <div class="row">
        <button class="btn pri" :disabled="busy || !rowsValid.length" @click="save">
          {{ busy ? '处理中…' : '保存到角色管理（' + rowsValid.length + ' 条）' }}
        </button>
        <span class="hint2">保存会<b>整体覆盖</b>上一次的外部统计（这是一份「此刻状态」的快照，不是追加）</span>
      </div>
      <div v-if="bad.length" class="badge-bad" style="font-size:12.5px;display:block">
        有 {{ bad.length }} 行没通过校验，<b>没有写入</b>：
        <div v-for="(b, i) in bad" :key="i">· {{ b.name }} —— {{ b.why }}</div>
      </div>

      <!-- ── 已录入 ──────────────────────────────────────────────────────── -->
      <h3 style="margin-top:18px">已录入的外部统计 <small v-if="conf.items">{{ conf.items }} 条 · 上传过 {{ conf.shots }} 张图</small></h3>
      <div class="ch">这份数据代表账号<b>此刻的真实持有状态</b>，重传一张更全的截图会把它整体顶掉（所以是覆盖，不是累加）。</div>
      <table class="tb" v-if="saved.items && saved.items.length">
        <thead><tr><th style="width:52px">图</th><th style="width:60px">类型</th><th>名字</th><th class="hn" style="width:96px">星魂/叠影</th><th style="width:170px">记录时刻</th></tr></thead>
        <tbody>
          <tr v-for="x in saved.items" :key="x.kind + x.id">
            <td><img class="mini" loading="lazy" :src="'assets/' + (x.kind === 'lc' ? 'light_cone' : 'avatar') + '/' + x.id + '.png'" :alt="x.name"></td>
            <td>{{ x.kind === 'lc' ? '光锥' : '角色' }}</td>
            <td>{{ x.name }} <code v-if="x.score != null">识别 {{ x.score.toFixed(3) }}</code></td>
            <td class="num">{{ x.v }}</td>
            <td class="mono">{{ String(x.at || '').slice(0, 19).replace('T', ' ') || '—' }}</td>
          </tr>
        </tbody>
      </table>
      <div v-else class="hint2">还没有录入任何外部统计 —— 角色管理页现在用的是「接口窗口 ＋ 截图补录」两源的推算值。</div>
      <div class="row" v-if="saved.items && saved.items.length">
        <button class="btn" :disabled="busy" @click="clearAll">清空全部外部统计</button>
        <span class="hint2">清空后角色管理页会退回两源推算值，截图留档不受影响。</span>
      </div>

      <!-- ── 与抽卡记录的冲突 ───────────────────────────────────────────── -->
      <div v-if="conf.conflicts && conf.conflicts.length" class="imp-conf">
        <b>⚠️ 有 {{ conf.conflicts.length }} 处「外部统计」与「抽卡记录推算」不一致</b>，已按外部统计生效：
        <div v-for="(c, i) in conf.conflicts" :key="i" class="cl">
          · <b>{{ c.name }}</b>（{{ c.unit }}）：外部说 <b>{{ c.ext }}</b>，抽卡记录推算 <b>{{ c.calc }}</b>
          <span class="d">{{ c.calcFrom }}</span>
        </div>
        <div class="d">不一致不等于谁错了：抽卡推算依赖「截图补录」那段手抄数据，缺一笔就会偏低；以外部统计为准最稳。</div>
      </div>
      <div v-if="conf.bad && conf.bad.length" class="badge-bad" style="font-size:12.5px;display:block">
        <b>data/external.json 里有 {{ conf.bad.length }} 行解析不出来</b>（已忽略，不影响其它条目）：
        <div v-for="(b, i) in conf.bad" :key="i">· {{ b.name }} —— {{ b.why }}</div>
      </div>
    </div>`,
  };
})();
